...
const DataStatsOne: React.FC<dataStats> = () => {
  const [parkingLotCount, setParkingLotCount] = useState<number>(0);
    ...
    fetch('http://localhost:8000/api/ParkingLots')
      ...
      .then(data => {
        if (data.status === 'OK' && Array.isArray(data.results)) {
          setParkingLotCount(data.results.length);
          const totalSpacesSum = data.results.reduce((sum: number, parking: any) => sum + parking.total_spaces, 0);
          setTotalSpaces(totalSpacesSum);
        } else {
  ...
  const dataStatsList = [
    ...
    },
    {
      ...
      color: "#FF9C55",
      title: "Tổng số lượng chỗ bãi đậu xe",
    ...
    },
    {
      ...
      color: "#8155FF",
      title: "Số lượng bãi đậu xe",
      value: parkingLotCount.toString(),
      growthRate: 2.59,
    },
    {
      icon: (
